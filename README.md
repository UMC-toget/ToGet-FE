...
간단 요약: A~E 화면에서 추출한 공통/페이지 전용 컴포넌트를 기준으로 UI 컴포넌트 목록과 구현 상태를 도출했습니다. 주요 내용은 프론트엔드 명세서(./프론트엔드-명세서.md)의 "공통 컴포넌트 목록 및 사용 방식"과 일치하며, 특히 E(펀딩 상세·참여)·H(함께 선물) 섹션에 사용되는 컴포넌트와 API 연동 흐름을 정리해 두었습니다. 일부 컴포넌트는 현재 페이지 전용으로 구현되어 있지만 재사용성이 확인되면 `components/common/`로 승격할 예정입니다.

예: Button, Input, PageLayout, BottomNav, Gift/Progress/Letter 관련 컴포넌트는 이미 구현되어 있으며, ImageUploader(사진 등록/크롭), StepIndicator 등 생성 플로우 관련 컴포넌트도 존재합니다. 상세한 매핑과 상태는 프론트엔드 명세서를 참고하세요.
