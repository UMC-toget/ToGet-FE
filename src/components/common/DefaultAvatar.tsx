...
interface DefaultAvatarProps {
  ...
  className?: string
  /** true면 리본 없이 연회색 원만 (참여자 목록 placeholder용) */
  plain?: boolean
}

/**
 * 기본 프로필 아바타.
 * - 기본: 그라데이션 원 배경 + 리본 로고 (프로필 이미지 없을 때 기본 프로필).
 * - plain: 연회색 원만 (참여자 목록처럼 리본 없이 placeholder만 필요할 때).
 *
 ...
 */
export default function DefaultAvatar({ className = 'size-full', plain = false }: DefaultAvatarProps) {
  if (plain) {
